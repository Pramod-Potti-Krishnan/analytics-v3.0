#!/bin/bash
# V2 Test: Advanced Chart Types (D3 + Chart.js plugins)
# Tests: treemap, heatmap, boxplot, candlestick, sankey, mixed
# Also tests D3 variants: d3_treemap, d3_sankey

ANALYTICS_URL="https://analytics-v30-production.up.railway.app"
LAYOUT_URL="https://web-production-f0d13.up.railway.app"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="./test_outputs/v2_advanced_charts_${TIMESTAMP}"

mkdir -p "$OUTPUT_DIR"

echo "=============================================="
echo "  V2 Test: Advanced Chart Types"
echo "=============================================="
...
# D3.js chart types (the only advanced charts enabled in production)
# Format: "analytics_type|chart_type|title"
# These 4 D3 charts are enabled in the production API
CHART_CONFIGS=(
    # D3-based charts (hierarchical/flow/geo data)
    "market_share|d3_treemap|D3 Treemap - Hierarchical Rectangles"
    "market_share|d3_sunburst|D3 Sunburst - Radial Hierarchy"
    "category_ranking|d3_choropleth_usa|D3 USA Map - Geographic Data"
    "market_share|d3_sankey|D3 Sankey - Flow Diagram"
)
...
for i in "${!CHART_CONFIGS[@]}"; do
    ...
    CHART_RESPONSE=$(curl -s -X POST "$ANALYTICS_URL/api/v1/analytics/L02/$ANALYTICS_TYPE?use_synthetic=true" \
        ...
        -d "{
            \"presentation_id\": \"v2-advanced-charts-test\",
            \"slide_id\": \"slide-$SLIDE_NUM\",
            \"slide_number\": $SLIDE_NUM,
            \"narrative\": \"Analysis showing $CHART_TYPE visualization with business performance data.\",
...
REQUEST_JSON="{
    \"title\": \"V2 Advanced Charts - $TIMESTAMP\",
    \"theme\": \"starter\",
    \"slides\": $SLIDES_JSON
}"
...
echo "URL: $PRES_URL"
echo ""
echo "Advanced Chart Types (V2-chart-text layout):"
echo ""
for i in "${!CHART_CONFIGS[@]}"; do
    CONFIG="${CHART_CONFIGS[$i]}"
    CHART_TYPE=$(echo "$CONFIG" | cut -d'|' -f2)
