"""Test D3.js choropleth USA map - geographic data visualization."""
import requests
import json
import os
...
def test_d3_choropleth(base_url: str, env_name: str):
    """Test d3_choropleth_usa chart type."""
    print(f"\n{'='*60}")
    print(f"Testing D3 Choropleth USA Map on {env_name.upper()}")
    print(f"{'='*60}\n")
    ...
    print(f"URL: {url}")
    print(f"Chart Type: d3_choropleth_usa")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "content" in result:
                ...
                # D3.js choropleth specific checks
                has_d3_cdn = "cdn.jsdelivr.net/npm/d3@7" in element_3
                has_topojson_cdn = "cdn.jsdelivr.net/npm/topojson-client@3" in element_3
                has_svg = "svg" in element_3.lower()
                has_choropleth = "choropleth" in element_3.lower()
                has_geo_projection = "geoAlbersUsa" in element_3
                has_us_atlas = "us-atlas" in element_3
                has_color_scale = "colorScale" in element_3 or "scaleQuantize" in element_3
                has_legend = "legend" in element_3.lower()

                print(f"D3.js Choropleth Indicators:")
                print(f"  ✓ Has D3.js v7 CDN: {has_d3_cdn}")
                print(f"  ✓ Has TopoJSON CDN: {has_topojson_cdn}")
                print(f"  ✓ Has SVG rendering: {has_svg}")
                print(f"  ✓ Has choropleth reference: {has_choropleth}")
                print(f"  ✓ Has geoAlbersUsa projection: {has_geo_projection}")
                print(f"  ✓ Has US Atlas TopoJSON: {has_us_atlas}")
                print(f"  ✓ Has color scale: {has_color_scale}")
                ...
                filename = f"test_d3_choropleth_{env_name}_{timestamp}.html"
...
<head>
    ...
    <title>D3 Choropleth Map Test - {env_name}</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <h1>D3 Choropleth USA Map Test ({env_name.upper()})</h1>

        <div class="metadata">
            <h3>Test Information</h3>
            <p><strong>Environment:</strong> {env_name}</p>
            <p><strong>Chart Type:</strong> d3_choropleth_usa</p>
            <p><strong>Data Points:</strong> {len(payload['data'])} states</p>
        ...
        <div class="metadata">
            <h3>Test Data</h3>
            <pre>{json.dumps(payload['data'], indent=2)}</pre>
        </div>
    </div>
</body>
</html>"""
                ...
                print(f"   Open this file in a browser to see the D3 choropleth map\n")

                # Validation summary
                all_checks = all([has_d3_cdn, has_topojson_cdn, has_svg, has_geo_projection, has_us_atlas, has_color_scale])
                if all_checks:
                    print(f"✅ ALL CORE CHECKS PASSED - D3 choropleth implementation validated!")
                else:
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("D3.JS CHOROPLETH USA MAP TEST")
    print("Testing D3.js geographic visualization")
    print("="*60)

    # Test local first (if available)
    print("\nAttempting local test first...")
    try:
        test_d3_choropleth(BASE_URLS["local"], "local")
    except:
        print("⚠️  Local server not running, skipping local test\n")

    # Always test production
    print("\nTesting production deployment...")
    test_d3_choropleth(BASE_URLS["production"], "production")
