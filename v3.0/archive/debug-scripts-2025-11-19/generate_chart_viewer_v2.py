...
Version 2: Properly loads all plugin CDN scripts in the head.
"""
...
def get_chart_html(chart_type, data, title):
    """Get the chart HTML from the API."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return result.get("content", {}).get("element_3", "")
    except:
        pass
    return f"<div style='color: red; padding: 20px;'>Failed to load {chart_type}</div>"


print("Generating chart viewer page v2...")
print("-" * 70)
...
print(f"\n✅ Chart viewer v2 created: {output_file}")
print(f"   File size: {len(page_html):,} bytes")
print(f"   All plugin CDN scripts loaded in <head>")
print(f"\n📂 Opening in browser...")
