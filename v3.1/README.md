...
**Version**: 3.8.0
**Status**: ✅ Production Ready - Synthetic Data Generation + D3.js Advanced Visualizations
**Railway**: [https://analytics-v30-production.up.railway.app](https://analytics-v30-production.up.railway.app)

A REST API analytics microservice that generates comprehensive charts and visualizations with Chart.js 4.4.0 and D3.js v7, providing interactive charts with AI-generated insights for presentation slides. Supports 18 chart types using native Chart.js capabilities and D3.js for advanced visualizations including geographic mapping, flow diagrams, and network graphs.

**🆕 NEW IN v3.8.0**: Independent synthetic data generation for all 18 chart types - test, develop, and preview charts without requiring external data sources.
...
- **Integration & API**
  - **[`DATA_FORMATS_REFERENCE.md`](DATA_FORMATS_REFERENCE.md) - Data format specifications for all 18 chart types** ⭐
  ...
  - [`docs/CHART_TYPE_CATALOG.md`](docs/CHART_TYPE_CATALOG.md) - All 18 chart types
...
The Analytics Microservice provides a REST API for generating interactive Chart.js and D3.js visualizations with AI-powered insights. Supports 18 chart types using native Chart.js and D3.js for reliable rendering. All charts are synchronous - no job polling required.
...
#### D3.js Chart Types (4) - Advanced Visualizations
- `d3_treemap` - Hierarchical treemap visualization using D3.js v7 (SVG-based)
- `d3_sunburst` - Radial hierarchical sunburst diagram using D3.js v7 (SVG-based)
- `d3_choropleth_usa` - Geographic choropleth map of USA states using D3.js v7 + TopoJSON (SVG-based)
- `d3_sankey` - Flow diagram for process/budget flows using D3.js v7 + d3-sankey plugin (SVG-based)
...
Keeping **18 total chart types**: 9 original + 5 new native Chart.js types + 4 D3.js types

### 📋 Data Format Reference

**IMPORTANT**: Different chart types require different data formats!

For complete data format specifications for all 18 chart types, see:
...
| `data` | array | ⚠️ Optional (v3.8.0+) | Chart data (2-50 points) | Auto-generated if missing |
| `chart_type` | string | ⚠️ Recommended (v3.8.0+) | Chart type for synthetic data | Based on analytics_type |
...
## 🎲 Synthetic Data Generation (NEW in v3.8.0)

**Feature**: Generate realistic, context-aware synthetic data for all 18 chart types without requiring external data sources.

### Overview

The Analytics Microservice can now independently generate synthetic data, enabling:
- ✅ **Independent Testing**: Test chart rendering without Director integration
- ✅ **Faster Development**: Preview chart types during development
- ✅ **Graceful Degradation**: Automatic fallback when data unavailable
- ✅ **Demo Mode**: Generate realistic data for demonstrations
- ✅ **Context-Aware**: Parses narratives to generate appropriate labels and values

### Three Ways to Use Synthetic Data

#### 1. Standalone Data Generation

Generate synthetic data without creating a slide:
...
Generate complete slide preview with synthetic data:
...
result = response.json()
# Returns complete slide with element_3 (chart) and element_2 (observations)
# Same response format as analytics endpoint
```

#### 3. Analytics with Synthetic Data

Use synthetic data in regular analytics endpoint (two ways):

**Option A: Explicit Synthetic Request**
```python
response = requests.post(
...
**Option B: Automatic Fallback** (when data field is missing)
```python
response = requests.post(
    ...
    json={
        ...
        # No data field - automatically uses synthetic data
    }
)
...
The synthetic data generator intelligently parses narratives:

**Example 1: Timeframe Detection**
```python
{
    "narrative": "Show quarterly revenue growth for 2024"
}
# Generated labels: "Q1 2024", "Q2 2024", "Q3 2024", "Q4 2024"
# Generated values: Upward trending amounts
```
...
- **18 Chart Types**: All Chart.js and D3.js chart types supported
- **15+ Business Scenarios**: revenue_growth, market_share, geographic_sales, etc.
- **Context Extraction**: Timeframe, trend, domain, magnitude, year
- **Chart-Specific Formatting**: US states for choropleth, flow notation for sankey, etc.
...
Generate standalone synthetic data.

**Request**:
```json
{
  "chart_type": "string",      // Required: One of 18 supported chart types
  "narrative": "string",        // Optional: Context for generation
  "num_points": integer,        // Optional: Number of data points (auto-determined if omitted)
  "scenario": "string"          // Optional: Business scenario (e.g., "revenue_growth")
}
```
...
{
  "success": true,
  "data": [...],                // Generated data in chart-specific format
  "metadata": {
    "chart_type": "string",
    "num_points": integer,
    "scenario": "string",
...
{
  "narrative": "string",        // Optional: Context for generation
  "num_points": integer         // Optional: Number of data points
}
```

**Response**: Same format as analytics endpoint (element_3, element_2, metadata)

### Backward Compatibility

**Zero breaking changes** - Existing Director integration continues to work:
...
- **[Synthetic Data Test Results](SYNTHETIC_DATA_TEST_RESULTS.md)** - Comprehensive test results (12/12 passing)
- **[Deployment Guide](DEPLOYMENT_READY_v3.8.0.md)** - Complete deployment documentation
- **[Director Integration](docs/DIRECTOR_INTEGRATION_SUMMARY.md)** - Updated Director integration guide
...
### D3.js Chart Types (2) - Added in v3.4.4+
- `d3_treemap` - Hierarchical treemap visualization using D3.js v7 for advanced data representation (SVG-based)
- `d3_sunburst` - Radial hierarchical sunburst diagram using D3.js v7 for multi-level data visualization (SVG-based)

### Plugin Chart Types - REMOVED (v3.4.3)
The following 8 plugin-based chart types were removed due to rendering issues:
- `treemap`, `heatmap`, `matrix`, `boxplot`, `candlestick`, `financial`, `sankey`, `mixed`

**Total: 16 working chart types** (9 original + 5 new native Chart.js types + 2 D3.js types)
